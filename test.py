...
def accessFunctional(block_addr, data, isWrite):
    ...
    if int(block_addr//blockSize) in cache_TLB:
        ...
        cache_TLB.move_to_end(int(block_addr//blockSize))
    if found:
        # Full Cache Search
...
def insert(address, data):
    ...
    while(current_level <= num_BTH):
        ...
        if(VBIR==0):
            x=1
            if(address//blockSize == 33558924):
                x=2
        # if (cache_DBA[145].tt.PT == 116 and isinstance(cache_DBA[145].data[19], BTH_entry) and (cache_DBA[488].data[19].I == 145 or cache_DBA[488].data[24].I == 145)):
        #     x=1
        if(len(cache_DBA[145].data) > 0 and isinstance(cache_DBA[145].data[19], BTH_entry) and cache_DBA[145].data[19].I == 145 and cache_DBA[145].data[24].I == 145):
            x=2
        ...
        if(cache_DBA[VBIR].dut.V == True and cache_DBA[VBIR].dut.LF > 0):
            ...
            if (cache_DBA[VBIR].dut.LF == num_BTH):
                if (cache_DBA[VBIR].tt.TAG in cache_TLB):
                    cache_TLB.pop(cache_DBA[VBIR].tt.TAG)
        ...
        if (current_level == 1):
            ...
            # Invalidate previous DBA entry pointed to by level N
            # if cache_DBA[last_BTH].data[int(address//(L0T_offset/pow(blockSize/2, current_level-1)))&int(blockSize/2-1)].V == True:
            #     x=1
    ...
    if (cache_DBA[last_BTH].tt.TAG in cache_TLB):
        x=1
...
for line in trace:
    ...
    if (not found):
        if(misses==5649):
            x=1
        data_BTH = insert(addr, data)
        # success, data2 = accessFunctional(addr, data2, False)
        data2 = cache_DBA[data_BTH].data[addr&int(blockSize-1)]
        assert(data==data2)
        misses += 1
